...
Meteor.startup(function () {
    ...
    Meteor.publish('feeds', function(){
        return Feeds.find({users:{$elemMatch: {user: this.userId}}});
    });
...
Accounts.onCreateUser(function(options,user){
    ...
    if(typeof options.services == 'undefined'){
        user.profile.avatar_url = Meteor.call("getgravatar",options.email);
    }
    return user;
});
/*Accounts.onCreateUser(function(options,user){
...
Meteor.methods({
   newfeed: function(feedurl){
       ...
       if(!feed){
           ...
           if(!_.contains(feed.users, this.userId)){
               Feeds.update(feed._id,{$push: {users: {user:this.userId, tags:[], folder:""}}});
   ...
   deletefeed: function(feedid){
       ...
       Feeds.update(feedid, {$pull: {users:{user: this.userId}}});
   ...
   refresh: function(feedid){
       ...
       if(!feedid){
           ...
           var uf = Feeds.find({users:{$elemMatch: {user: this.userId}}});
           console.log("Numberof feeds: "+uf.count());
           uf.forEach(function(feed){
                var diff = moment().diff(moment('"'+feed.updated+'"',"X"), 'minutes');
                if(diff>30){
                    console.log("Getting feed: "+feed.feedname+" last time checked "+diff+" minutes ago");
                    ...
                    if(fs.error){
                       ...
                       Meteor.call("refreshposts",feed,entries);
                       Feeds.update(feed._id, {$set:{updated: moment().unix()}});
                    }
                }else
                    console.log('Checked '+diff+' minutes ago');
           });
       }else{
   ...
   refreshposts: function(feed,entries){
